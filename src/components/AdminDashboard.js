...
import { DEFAULT_GENRE } from '../constants/movieGenres';
...
const AdminDashboard = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.movieType === 'premium' && formData.price < 200) {
      setError('Minimum price for premium movies is 200 TSHS');
      return;
    }
    ...
    try {
      ...
      // Reset form
      setFormData({ movieName: '', downloadLink: '', category: DEFAULT_CATEGORY, posterImage: '', movieType: 'free', price: 0, description: '' });
      setPosterFile(null);
      setPosterPreview('');
